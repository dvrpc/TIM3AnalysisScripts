...
base_path = r'D:\TIM3.1\CalibrationIdesOfMarch2021\scenario\DaySimSummaries\data'
hh_file = os.path.join(base_path, 'dvrpc_hrecx6.dat')
per_file = os.path.join(base_path, 'dvrpc_precx6.dat')
tour_file = os.path.join(base_path, 'dvrpc_tourx6.dat')
...
print('Merging')
data = hh.merge(per, on = 'hhno').merge(tour, on = ['hhno', 'pno']).query('hhtaz < 50001 and tautodist >= 0')
...
for p in purps:
    ...
    dists.loc[purps[p]] = np.average(purp_data['tautodist'], weights = purp_data['psexpfac'])
