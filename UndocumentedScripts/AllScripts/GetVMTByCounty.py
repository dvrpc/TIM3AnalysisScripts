...
for year in [2015, 2040]:
    ...
    trip[year]['vmt'] = trip[year]['trexpfac'] * trip[year]['travdist']
    trip[year]['ocounty'] = trip[year]['otaz'].map(taz2county)
    trip[year]['dcounty'] = trip[year]['dtaz'].map(taz2county)
    o_by_county[year] = trip[year].groupby('ocounty').sum()['vmt']
    d_by_county[year] = trip[year].groupby('dcounty').sum()['vmt']

print('Writing')

outfile = r'D:\TIM3\VMTByCounty1540.xlsx'
