...
base_path = r'Y:\TIM_3.1\DVRPC_ABM_Testing\scenario'
trip_file = os.path.join(base_path, 'Output', '1023_SA_max30_wkwt125', '_trip_2.dat')
...
outfile = os.path.join(r'D:\TIM3\DSTrTripsByAvailModeCounty_Halfmile_{}.csv'.format(date_label))
...
maz2sa = pd.read_csv(maz2sa_file, delimiter = ' ').query('distance < 2640')
...
#trip[cols].query('o_Pat == 1 or d_Pat == 1').to_csv(outfile_sub)
#trip[cols].query('o_Rail == 1 or d_Rail == 1').to_csv(outfile_sub)
