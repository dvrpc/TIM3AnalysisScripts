...
infile = r'D:\TIM3\TransitTripsWithBestSAs_200102.csv'
outfile = r'D:\TIM3\StopAreaChoiceSummary_200102_BusIso.xlsx'
sa2mode_file = r'D:\ref\sa2mode.csv'
sa2mode_file2 = r'D:\ref\sa2mode_w_stops.csv'
...
trip['ozone'] = trip['opcl'].map(maz2taz)
trip['dzone'] = trip['dpcl'].map(maz2taz)
trip = trip.query('ozone in @subway_tazs and dzone in @subway_tazs')

print('Filtering for bus-iso to bus-iso')
sa2mode_stops = pd.read_csv(sa2mode_file2)
sa2mode_stops['bus-iso'] = (sa2mode_stops['BUS'].astype(bool) & ~(sa2mode_stops['SUBINSTOP'].astype(bool)))
iso_bus_sas = list(sa2mode_stops[sa2mode_stops['bus-iso']]['$STOPAREA:NO'])
trip = trip.query('otaz in @iso_bus_sas and dtaz in @iso_bus_sas')
...
trip.to_csv(outfile.replace('.xlsx', '.csv'))
