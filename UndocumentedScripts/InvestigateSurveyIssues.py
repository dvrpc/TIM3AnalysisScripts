...
def get_transit_time(args):
    '''
    Reads in a skim value from a given od pair

    Parameters
    ----------
    args (tuple):
        Length-2 tuple where the first element is the origin zone number and the second element is the destination zone number

    Returns
    -------
    Value (float):
        Value of the skim at the given origin and destination. If there is a key error, None is returned
    '''
    global skim
    try:
...
transit_tours = tables['tour'].query('totaz > 0 and tdtaz > 0 and (tmodetp == 6 or tmodetp == 7)') #Identify which tours use a mode of transit
transit_tours['od'] = list(zip(transit_tours['totaz'], transit_tours['tdtaz'])) #Zip origins and destinations into tuples
transit_tours['AMTransitTime'] = transit_tours['od'].apply(get_transit_time) #Gets the transit time from the skim
transit_tours['Invalid'] = (transit_tours['AMTransitTime'] == 999999) #Identifies which tours have invalid transit times
#invalid_tours = transit_tours.query('Invalid')

transit_tours['tourid'] = list(zip(transit_tours['hhno'], transit_tours['pno'], transit_tours['day'], transit_tours['tour'])) #Zip together information identifying tour

transit_tour_trips = transit_tours.merge(tables['trip'], on = ['hhno', 'pno', 'day', 'tour']) #Merge transit tour table to trip table
transit_tour_trips['tourid'] = list(zip(transit_tour_trips['hhno'], transit_tour_trips['pno'], transit_tour_trips['day'], transit_tour_trips['tour'])) #Zip together information identifying tour
trips_by_tour_and_mode = transit_tour_trips.groupby(['tourid', 'mode']).count()['trexpfac'].reset_index().pivot('tourid', 'mode', 'trexpfac').fillna(0) #Count number of trips by mode in each tour
